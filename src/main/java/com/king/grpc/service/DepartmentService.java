...
public class DepartmentService extends DepartmentServiceGrpc.DepartmentServiceImplBase {
    ...
    public void listDepartments(Empty request, StreamObserver<ListDepartmentsResponse> responseObserver) {
        ...
        responseObserver.onNext(this.mapToResponse(all));
        responseObserver.onCompleted();
    }
    ...
    private Department mapToDeptResponse(DepartmentEntity department) {
    ...
    private ListDepartmentsResponse mapToResponse(List<DepartmentEntity> department) {
        return ListDepartmentsResponse.newBuilder()
                .addAllDepartments(department.stream().map(this::mapToDeptResponse).toList())
                .build();
    }
}